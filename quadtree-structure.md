...
<p>Particles are binned into voxels of size `Dw^2 x Dh`, where `Dw` is the bin's width and `Dh` is the bin's depth. These dimensions are constant throughout the entire domain. However, areas of high particle density could benefit from being further refined for greater accuracy and decreasing $Dw$ may not always be possible memory-wise.<br>
